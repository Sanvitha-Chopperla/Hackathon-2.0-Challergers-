...
import { Building2, QrCode, ShieldCheck, ShieldAlert } from 'lucide-react';
import { verifyCertificate } from '../services/blockchain';

export default function EmployerPortal() {
    const [isAuthenticated, setIsAuthenticated] = useState(false);
    const [scanResult, setScanResult] = useState<{ isValid: boolean, error?: string, data?: any } | null>(null);
    ...
    const handleScan = (result: any) => {
        if (!result) return;
        try {
            // react-qr-scanner returns an array of objects: [{ rawValue: "..." }]
            const textData = Array.isArray(result) && result.length > 0 ? result[0].rawValue : result;
            if (!textData) return;
            ...
            if (data.hash) {
                ...
                // Check if QR code is fresh (generated within the last 60 seconds)
                const isFresh = data.timestamp ? (Date.now() - data.timestamp < 60000) : false;

                if (!isFresh && data.timestamp) {
                    setScanResult({ isValid: false, error: 'Expired QR Code. The candidate must present a freshly generated code to prevent replay attacks.', data });
                    return;
                }

                const isValid = verifyCertificate(data.hash);
                setScanResult({ isValid, data });
            }
        } catch (e) {
            // Invalid QR format for our system
            console.error(e);
            setIsScanning(false);
            setScanResult({ isValid: false, error: 'Invalid QR Format.' });
        }
    };
            ...
            <div className="max-w-md mx-auto mt-12 bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                    <div>
                        ...
                        <input required type="email" placeholder="hr@company.com" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-all" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Password</label>
                        <input required type="password" placeholder="••••••••" className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 outline-none transition-all" />
                    </div>
                        ...
                        {scanResult.isValid ? (
                            <>
                                ...
                                <div className="bg-white p-4 rounded-xl border border-green-100 mt-4 w-full text-left">
                                    ...
                                    <p className="mb-2"><span className="text-gray-500">Student ID:</span> <span className="font-bold text-gray-900">{scanResult.data?.studentId}</span></p>
                                    <p className="mb-2 overflow-hidden"><span className="text-gray-500">Ledger Hash:</span><br /><span className="font-mono text-xs text-green-700 break-all">{scanResult.data?.hash}</span></p>
                                    {scanResult.data?.uniqueId && (
                                        <>
                                            <div className="border-t border-green-200 my-2"></div>
                                            <p className="mb-1 text-xs"><span className="text-gray-500">Session ID:</span> <span className="font-mono text-gray-700">{scanResult.data.uniqueId.split('-')[0]}...</span></p>
                                            <p className="mb-0 text-xs"><span className="text-gray-500">Gen Time:</span> <span className="text-gray-700">{new Date(scanResult.data.timestamp).toLocaleTimeString()}</span></p>
                                        </>
                                    )}
                                </div>
                            ...
                            <>
                                ...
                                <h2 className="text-3xl font-extrabold text-red-900 mb-2">VERIFICATION FAILED</h2>
                                <p className="text-red-800 mt-2 font-medium bg-red-200/50 px-4 py-2 rounded-lg max-w-sm">
                                    {scanResult.error || "WARNING: This credential could not be found on the cryptographic ledger. This indicates potential fraud or manipulation."}
                                </p>
                            </>
                        )}
